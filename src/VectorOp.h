...
public:
	...
	static vector<T> GetRandom(int iNum, int iStart, int iEnd);
	static void Swap(vector<T>& V, int iLeftIdx, int iRightIdx );
};
 ...
 * Created By 	: hrushi
 * Comments		: Swaps the index values in the array
 * Arguments	: vector<T>& V, int iLeftIdx, int iRightIdx
 *******************************************************************/
template<typename T>
void VectorOp<T>::Swap( vector<T>& V, int iLeftIdx, int iRightIdx )
{
	T Temp = V.at(iLeftIdx);
	V.at(iLeftIdx) = V.at(iRightIdx);
	V.at(iRightIdx) = Temp;
}
 ...
 * Created On	: May 7, 2014
 * Created By 	: hrushi
 * Comments		: Generate iNum elements from iStart to iEnd
 * Arguments	: int iNum, int iStart, int iEnd
 *******************************************************************/
template<typename T>
vector<T> VectorOp<T>::GetRandom( int iNum, int iStart, int iEnd )
{
...
std::istream& operator>>( std::istream& in, vector<T>& vec)
{
	...
	while(( iValidField < RequiredFileds ) || bTillEoFLine )
	{
		...
		try
		{
			...
			bool b = File::isSkipChar(TempStr);
